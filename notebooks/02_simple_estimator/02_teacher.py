...
@app.cell
def _(GradientBoostingClassifier, df2, features_1):
    model = GradientBoostingClassifier(random_state=512).fit(X=df2[features_1], y=df2['churn'])
    return (model,)
...
@app.cell
def _(df_treat, features_1, model):
    # Let's get the probability of churn from these predictions
    treat_churn_prob = model.predict_proba(df_treat[features_1])[:, 1].mean()
...
@app.cell
def _(df_ctrl, features_1, model):
    # Let's get the probability of churn from these predictions
    ctrl_churn_prob = model.predict_proba(df_ctrl[features_1])[:, 1].mean()
