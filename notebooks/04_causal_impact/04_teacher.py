...
@app.cell
def _(fit_causalimpact, streaming_data, summary):
    # Define periods
    pre_period_stream = [streaming_data.index[0], streaming_data.index[18]]
    post_period_stream = [streaming_data.index[19], streaming_data.index[39]]

    # Fit the model
    impact_streaming = fit_causalimpact(streaming_data, pre_period_stream, post_period_stream)

    # Print summary
    print(summary(impact_streaming))
    return impact_streaming, post_period_stream, pre_period_stream


@app.cell
def _(impact_streaming, plot):
    # Visualize results
    plot(impact_streaming)
    return
